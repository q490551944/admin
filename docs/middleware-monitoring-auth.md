...
访问 `/monitor` 或 `/monitor/` 会进入 `/monitor/index.html`。未登录时跳转到独立的 `/monitor/login.html`，使用现有员工用户名和密码登录。认证成功后再次确认白名单权限；无权限显示拒绝页面，认证服务暂不可用显示重试和退出操作，监控关闭显示未启用。页面不会改用模拟身份或模拟数据。

登录和退出使用同源 Cookie，每次写请求前重新读取 CSRF。页面不将密码、会话 ID 或 CSRF 写入浏览器存储，退出清除监控身份，并通过同源 BroadcastChannel 通知其他监控标签重新验证。通知仅包含失效事件；刷新、历史恢复或重新获得焦点也会重新检查身份。页面支持部署在 servlet context-path 下，跳转保持在同一监控目录内。此阶段只交付账号入口，指标概览由后续任务接入。

浏览器验收方法和产物见 [监控登录浏览器回归](middleware-monitoring-browser-tests.md)。
