...
`CollectionRequest` 是服务端对象，持有已有客户端、有效配置、单个绑定的授权范围，以及目标、实际来源 `source`、稳定绑定标识 `bindingId`、普通/容量类别、身份代次、序号、计划时间和截止时间。`bindingId` 取 `SourceBinding.targetId`，`source` 保留实际客户端 bean 名。同一 bean 可对应多个独立授权范围，不能仅凭 bean 名合并采集状态。截止时间从计划入队时起算，包含等待；采集器不得关闭借用的业务客户端。调度器把每个 `SourceBinding` 自己的范围传入，不能传合并目录的范围并集。

请求还携带服务端 `CollectionControl`，供调度器协调取消、结果提交与计数基线。请求类型及全部字段禁止 JSON 暴露，`toString` 仅含安全引用和序号。原有构造器默认 `bindingId=source`，并创建无线程的 detached control，保留既有调用兼容性。

`CollectionResult` 返回开始/结束时间、采集状态、指标样本、可选的独立服务探测、`inventoryComplete` 及可空的整次采集 `reason`。例如首次采集尚未发现任何指标就超时，可以返回 `FAILED`、`TIMEOUT`、空指标和空探测；不能伪造指标或将超时直接当作连接失败。`Attempt` 同样记录该原因。旧构造器默认原因为空；`PARTIAL` 可附带任一种具体缺失原因，`SUCCESS` 不带失败原因。

普通和容量任务是不同的采集序列，每个绑定也各自独立；一个绑定的成功不会覆盖另一绑定的失败。`inventoryComplete=true` 表示返回的指标身份集合是该绑定、类别、授权范围的完整清单，允许退役不再存在的节点/指标。它不代表每项数值都成功：已知仍存在但采集失败的指标必须返回缺失占位，以保留最近成功值。无法确认资源清单完整时使用默认 `false`，不能靠一次 `SUCCESS` 推断清单完整。完整空清单只清除对应绑定和类别，不影响其他绑定或其他采集类别。
...
`MonitoringCounterStore` 每个计算系列只留一份基线；系列键必须包含绑定、采集类别和原生对象范围，身份代次放在样本值中，换连接不会不断增加键。目标上限来自 `monitor.max-targets`，每目标最多 5000 个计数系列；超限返回 `BUSY`。键最长 2048 字符，不能包含控制字符。
...
`MonitoringSnapshotStore` 每目标最多保留 5000 个当前指标，绑定上限为 `monitor.max-targets`；普通/容量及绑定分别记序号。多个绑定即便借用同一 bean，也分别占用绑定容量。新增目标/绑定/指标超限时整次更新被拒绝，不留下半份结果。完整清单可退役旧资源，为节点替换释放空间。配置身份代次切换清空当前快照，删除目标和清空操作释放状态。

调度器先激活目标代次，接纳时拒绝旧代次、重复/倒退序号、倒退采样时间、超过截止时间和无效时间范围。`StoredMetric` 和 `Attempt` 同时保留实际 `source` 与 `bindingId`，`serviceProbes` 按 `bindingId` 索引；同采样时间的已接受新结果能够替换同绑定的旧结论。调度器必须在写入计数基线前也拒绝退役代次/超时结果；改变某个绑定实际借用的客户端时切换目标代次。重启后内存清空，不创建指标表、历史接口或趋势记录。
